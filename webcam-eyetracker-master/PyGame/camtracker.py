...
class Setup:
	...
	def setup_GUI(self):
		...
		# find image paths
		imgpaths = {}
		buttnames = ['1','2','3','4','up','down','t','start','escape']
		...
		buttpos['1'] = int(self.dispsize[0]*(2/6.0) - buttsize[0]/2), y #-->2
		buttpos['2'] = int(self.dispsize[0]*(3/6.0) - buttsize[0]/2), y #-->3
		buttpos['3'] = int(self.dispsize[0]*(4/6.0) - buttsize[0]/2), y #-->close eyes for eight seconds
		buttpos['4'] = int(self.dispsize[0]*(5/6.0) - buttsize[0]/2), y #-->left and right move eyes
		...
		buttpos['start'] = leftx, self.dispsize[1]/2 # halfway snapshot (==halfway display), to the left  -->start
	...
	def draw_stage(self, stagenr=None):
		...
		# universal buttons
		buttonstodraw = ['1','2','3','4','escape','t','start']
		activetodraw = []
		...
		if stagenr == 1:
			...
			title = "close eyes for 8 seconds"
			buttonstodraw.extend(['up','down'])
	...
	def run_GUI(self):
		...
		stagevars[4] = {}
		stagevars[4]['confirmed'] = False  #8 second eye
		stagevars[4]['y'] = 0 
		stagevars[4]['start_checked'] = False
		
		stagevars[5] = {}
		stagevars[5]['confirmed'] = False  #right and left
		...
		while running:
			...
			self.settings['threshold'] = 40
			stagevars[1]['thresholdchange'] = None
			
			# # # # #
			# stage specific
			
			# stage 1: select eye by clicking on it
			if stage == 1:
				# set camera threshold
			...
			# stage 2: confirmation + close eyes
			if stage == 2:
				...
				#time.sleep(3)
				#str_pupilrect = str(self.settings['pupilrect']).split(",") # <rect(311, 177, 100, 50)>
				#tmp = str_pupilrect[1]
				#stagevars[4]['y'] = int(tmp) #int type
				#print stagevars[4]['y']
				#print stagevars[4]['start_checked']
				#if stagevars[4]['start_checked']:
				...
				for i in range(0,len(record_y)):
					if i < len(record_y)/3:
						one_third = one_third+record_y[i]
					else:
						two_third = two_third+record_y[i]
				tmp = one_third*2
				print str(tmp) + " " + str(two_third)
				elapsed_time = 0
				if two_third > tmp:
					if first_in == 0:
						first_in = 1
				...
				if elapsed_time > 10.0:
					stagevars[4]['confirmed'] = True
					winsound.PlaySound("./resources/123.wav", winsound.SND_ALIAS)
					time.sleep(0.4)
					stage = 3
				#if y - stagevars[4]['y'] > 0:
					#print "yes!"
				#for i in range(0,10):
					#print "2: " + str(y)
					#time.sleep(7)
					#print time.time()
					#winsound.PlaySound("./resources/clock (mp3cut.net).wav", winsound.SND_ALIAS)
					#play music
				#time.sleep(5)
				#elapsed_time = time.time() - start_time
				#print "elapsed_time: " + str(elapsed_time)
				#if (elapsed_time) > 8.0:
					#tagevars[4]['confirmed'] = True
				
				# is settings are confirmed, stop running
				#if stagevars[4]['confirmed']:
					#running = False
			
			# stage 3: left and right move eyes
			if stage == 3:
				print "4"

			
			# stage 4: left and right move eyes
			#if stage == 4:
	...
	def handle_input(self, inptype, inp, stage, stagevars):
		...
		if inp == 'start':
			if stagevars[4]['start_checked']:
				...
				stagevars[4]['start_checked'] = True
